...
`xlim`. Pero para eso usamos `st_bbox()` que retorna la extensión del
objeto geográfico:
...
Como vemos, hay 2 núcleos principales y algunos puntos que representan,
probablemente errores de avistamiento (?). Debieramos removerlos antes
del análisis.

### Visualización interactiva

Podemos usar mapview para explorar los datos de forma interactiva,
usando la columna `year`, por ejemplo, para ver como se ha muestreado a
...
localidad y año, luego crea (con `mutate()`) una columna donde almacena el número de
registros distintos.
...
Finalmente podemos guardar las tabla a un archivo CSV para poder subirlo
a excel o qgis después, en caso de necesidad.

```{r}
# st_write(conteo, paste0(getwd(),"/","conteo.csv"), driver = "CSV", layer_options = "GEOMETRY=AS_XY", delete_dsn=TRUE)
```
...
las coordenadas en que hay registros de *C. patagonus* en GBIF.
...
### Selección de vaiables (bioclimaticas)
...
rm(Bioclim) # borramos para aliviar la memoria del compu!
```
