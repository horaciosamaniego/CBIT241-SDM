...
Vamos a hacer un modelo de distribucion de especies para *Cyanoliseus patagonus*.
[Aquí](https://www.avesdechile.cl/059.htm) una descripción del loro tricahue.
...
Un ejemplo: ¿Cuántos registros georeferenciado de tricahue (*Cyanoliseus patagonus*) existen en GBIF?
...
Usando el número de registro para *Cyanoliseus patagonus* en la Base de datos GBIF podemos contarlos:
...
Vemos (con: `dim(cp$data)`) que la consulta nos retornó `r nrow(cp$data)` registros (filas) y `r ncol(cp$data)` columnas
...
Clima2 <- raster::extract(Bioclim2, cp_sf) |>
    as.data.frame()

#rm(Bioclim)
```

Unimos los datos de ocurrencia de *C. patagus* con los datos biolimáticos para tener todo en una sola tabla que
